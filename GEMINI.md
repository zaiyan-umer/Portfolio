...
### Route groups
- `app/(frontend)/**` — all public-facing pages (home, projects, education, repositories). The group has its own `template.tsx` that applies a fade/slide transition on route change via `motion`.
